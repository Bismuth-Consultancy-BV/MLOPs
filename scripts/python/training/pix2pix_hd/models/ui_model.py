...
class UIModel(BaseModel):
    ...
    def change_labels(self, click_src, click_tgt):
        ...
        if idx_src.shape:
            ...
            self.label_map[
                idx_src[:, 0], idx_src[:, 1], idx_src[:, 2], idx_src[:, 3]
            ] = label_tgt
            self.net_input[
                idx_src[:, 0], idx_src[:, 1] + label_src, idx_src[:, 2], idx_src[:, 3]
            ] = 0
            self.net_input[
                idx_src[:, 0], idx_src[:, 1] + label_tgt, idx_src[:, 2], idx_src[:, 3]
            ] = 1

            # update the instance map (and the network input)
            if inst_tgt > 1000:
                # if different instances have different ids, give the new object a new id
                tgt_indices = (self.inst_map > label_tgt * 1000) & (
                    self.inst_map < (label_tgt + 1) * 1000
                )
    ...
    def add_strokes(self, click_src, label_tgt, bw, save):
        ...
        for i in range(bw):
            idx_src[i * bw : (i + 1) * bw, 2] = min(
                h - 1, max(0, click_src[0] - bw // 2 + i)
            )
            for j in range(bw):
        ...
        if idx_src.shape:
            ...
            for k in range(self.opt.label_nc):
                self.net_input[
                    idx_src[:, 0], idx_src[:, 1] + k, idx_src[:, 2], idx_src[:, 3]
                ] = 0
            self.net_input[
                idx_src[:, 0], idx_src[:, 1] + label_tgt, idx_src[:, 2], idx_src[:, 3]
            ] = 1
    ...
    def set_features(self, idx, feat, cluster_idx):
        for k in range(self.opt.feat_num):
            self.feat_map[idx[:, 0], idx[:, 1] + k, idx[:, 2], idx[:, 3]] = feat[
                cluster_idx, k
            ]
    ...
    def get_current_visuals(self, getLabel=False):
        ...
        if self.mask is not None:
            mask = np.transpose(self.mask[0].cpu().float().numpy(), (1, 2, 0)).astype(
                np.uint8
            )
