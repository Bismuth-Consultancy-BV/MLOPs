...
def run(model, input_colors, points, torch_device, local_cache_only):
    ...
    if len(points) == 0:
        ...
        sam_model = SamModel.from_pretrained(model).to(torch_device)
        processor = SamProcessor.from_pretrained(model)

        inputs = processor(image, input_points=[points], return_tensors="pt").to(torch_device)
        with torch.no_grad():
