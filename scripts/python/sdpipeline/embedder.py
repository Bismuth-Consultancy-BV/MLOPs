...
def run(
    input_ids_positive, input_ids_negative, torch_device, model, local_cache_only=True
):
    try:
    ...
    with torch.no_grad():
        input_ids_positive = torch.from_numpy(
            numpy.array([input_ids_positive], dtype=numpy.int64)
        )
        positive_text_embeddings = text_encoder(input_ids_positive.to(torch_device))[0]

        input_ids_negative = torch.from_numpy(
            numpy.array([input_ids_negative], dtype=numpy.int64)
        )
        negative_text_embeddings = text_encoder(input_ids_negative.to(torch_device))[0]

    all_text_embeddings = (
        torch.cat([negative_text_embeddings, positive_text_embeddings])
        .detach()
        .cpu()
        .numpy()
    )
